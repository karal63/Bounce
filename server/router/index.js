...
router.get('/logout', verifyToken, (req, res, next) => auth.logout(req, res, next));

router.get('/refresh', (req, res, next) => auth.refresh(req, res, next));

router.get('/activate/:link', (req, res, next) => auth.activate(req, res, next));
...
router.get('/messages/:userId/:type/:recipientId', verifyToken, (req, res, next) =>
...
router.get('/groups/:userId', verifyToken, (req, res, next) => group.getGroups(req, res, next));
...
router.post('/join-group/:link', verifyToken, (req, res, next) => group.joinGroup(req, res, next));
...
router.post('/ban-member', verifyToken, (req, res, next) => member.banMember(req, res, next));
...
router.post('/upload-image', verifyToken, upload.single('image'), (req, res, next) =>
...
router.patch('/user', verifyToken, (req, res, next) => user.update(req, res, next));

router.get('/reactions/:roomId', verifyToken, (req, res, next) =>
    message.getReactions(req, res, next)
);

router.get('/stickers', verifyToken, (req, res, next) => sticker.get(req, res, next));

router.post('/add-reaction', verifyToken, (req, res, next) => reaction.add(req, res, next));
