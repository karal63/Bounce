...
class GroupService {
    ...
    async create(name, ownerId, description) {
        const [nameRows] = await db.query('SELECT 1 FROM `groups` WHERE name = ? LIMIT 1', [name]);
        ...
        await db.query('INSERT INTO members (id, groupId, userId, role) VALUES (UUID(), ?, ?, ?)', [
            groupId,
            ownerId,
            'admin',
        ]);
    ...
    async join(link, userId) {
        ...
        const memberId = v4();
        await db.query('INSERT INTO members (id, groupId, userId, role) VALUES (?, ?, ?, ?)', [
            memberId,
            groupId,
            userId,
            'member',
    ...
    async delete(groupId, userDto) {
        ...
        const [groupRows] = await db.query('SELECT * FROM `groups` WHERE id = ? AND ownerId = ?', [
            groupId,
            user.id,
        ]);
        if (groupRows.length === 0)
            throw ApiError.BadRequest('You have no permission to delete this group.');

        await db.query('DELETE FROM `groups` WHERE id = ? AND ownerId = ?', [groupId, user.id]);
    }

    async leave(groupId, userDto) {
        const user = await userService.getUser(userDto);
        await db.query('DELETE FROM members WHERE groupId = ? AND userId = ?', [groupId, user.id]);
    }

    async rename(id, newGroupName) {
        await db.query('UPDATE `groups` SET name = ? WHERE id = ?', [newGroupName, id]);
    }
}
