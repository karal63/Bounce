<script setup lang="ts">
    ...
    onMounted(() => {
        if (!props.messageContext.message?.senderId || !sessionStore.user?.id) return;

        if (currentChatStore.currentRoom.type === 'group') {
            const message = props.messageContext.message;

            const receiver = findMemberById(currentChatStore.members, sessionStore.user?.id);

            if (sessionStore.user?.id === message.senderId || hasPermissions(receiver)) {
