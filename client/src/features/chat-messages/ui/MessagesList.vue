<script setup lang="ts">
    ...
    const handleNewMessage = ({
        ...
        if (room.type === 'group' && newMessage.groupId === room.id) {
            ...
            currentChatStore.attachments = [...currentChatStore.attachments, ...attachments];
    ...
    const handleDeleteMessage = (messageId: string) => {
        currentChatStore.messages = currentChatStore.messages.filter(msg => msg.id !== messageId);
    };

    const handleNewReaction = (reaction: Reaction) => {
        const index = currentChatStore.reactions.findIndex(
            r => r.messageId === reaction.messageId && r.stickerId === reaction.stickerId
        );
    ...
    const handleDeleteReaction = (reaction: Reaction) => {
        currentChatStore.reactions = currentChatStore.reactions.reduce((acc: Reaction[], r) => {
            if (r.messageId === reaction.messageId && r.stickerId === reaction.stickerId) {
    ...
    const showContext = (
        ...
        const posX = buttonRect.left - containerRect.left + listRef.value.scrollLeft;
...
<template>
    <div ref="listRef" class="relative h-[90%] mb-2 flex justify-center overflow-y-auto">
