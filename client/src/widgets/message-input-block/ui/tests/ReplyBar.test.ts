...
vi.mock('@/features/reaction', async () => {
    ...
    const actual =
        await vi.importActual<typeof import('@/features/reaction')>('@/features/reaction');
...
describe('ReplyBar', () => {
    ...
    it('shows reply button when hovering', async () => {
        ...
        await wrapper.vm.$nextTick();
        const replyButton = singleMessageWrapper.find('[data-testid="reply-button"]');
        ...
        expect(inputWrapper.find('[data-testid="reply-name"]').text()).toBe('mocked_userName');
